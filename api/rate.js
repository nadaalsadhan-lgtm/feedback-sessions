...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (url && authToken) {
      ...
      // A new rating supersedes any old follow-up comment for this client+metric.
      // Clear the stored comment so stale feedback doesn't stick to a changed score.
    ...
    '<script>' +
      'function submitFb(e){e.preventDefault();' +
        ...
        'fetch("/api/feedback",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)})' +
          '.then(function(){document.getElementById("ask").style.display="none";' +
            'document.getElementById("done").style.display="block";})' +
          '.catch(function(){document.getElementById("ask").style.display="none";' +
            'document.getElementById("done").style.display="block";});' +
        'return false;}' +
    '</script>';
  return shell(inner);
