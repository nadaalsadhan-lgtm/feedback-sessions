...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Build a lookup of each client's latest rating time per metric,
    // so a comment only counts if it belongs to that current rating (not an old one).
    var latestRatingAt = {}; // key: client|metric -> ISO time of newest rating
    for (var d = 0; d < dedupAll.length; d++) {
      var dr = dedupAll[d];
      var dk = (dr.client || '') + '|' + (dr.metric || 'onboarding_effort');
      if (!latestRatingAt[dk]) latestRatingAt[dk] = dr.ratedAt || '';
    }
    ...
    for (var k = 0; k < cRaw.length; k++) {
      try {
        ...
        // Skip comments older than the client's current rating for this metric.
        // (A newer rating supersedes old feedback, so it shouldn't resurface.)
        var rk = cm.client + '|' + mm;
        var ratingTime = latestRatingAt[rk] || '';
        var commentTime = cm.at || '';
        if (ratingTime && commentTime && commentTime < ratingTime) continue;
